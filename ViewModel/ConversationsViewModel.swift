...
class ConversationsViewModel: ObservableObject {
    ...
    private var recentMessagesDirectory = [String: Message]()
        ...
        query.addSnapshotListener { snapshot, error in
            ...
            changes.forEach { change in
                ...
                COLLECTION_USERS.document(uid).getDocument { snapshot, _ in
                    ...
                    self.recentMessagesDirectory[uid] = Message(user: user, dictionary: messageData)
                    self.recentMessages.append(Message(user: user, dictionary: messageData))
                    
                    self.recentMessages = Array(self.recentMessagesDirectory.values)
                }
            }
        }
